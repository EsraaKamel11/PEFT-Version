...
try:
    ...
    logger.warning("Prometheus monitoring not available. Install with: pip install prometheus-fastapi-instrumentator prometheus-client")
    ...
    async def get_model(self, base_model, adapter_version):
        ...
        try:
            ...
            if PROMETHEUS_AVAILABLE:
                ...
                MODEL_MEMORY_USAGE.labels(model=base_model, version=adapter_version).set(model_size)
            ...
            if PROMETHEUS_AVAILABLE:
                ...
                MODEL_LOAD_TIME.labels(model=base_model, version=adapter_version).observe(load_time)
