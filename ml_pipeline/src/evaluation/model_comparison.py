...
class ModelEvaluator:
    ...
    def load_model(self, model_path: str, is_peft: bool = False) -> Tuple[Any, Any]:
        ...
        try:
            if is_peft:
                ...
                base_model_name = "microsoft/DialoGPT-medium"  # Can be made configurable
    ...
    def generate_response(self, model: Any, tokenizer: Any, question: str, 
    ...
    def calculate_bleu_score(self, predictions: List[str], references: List[str]) -> float:
        ...
        try:
            ...
            for pred, ref in zip(predictions, references):
                ...
                score = sentence_bleu([ref_tokens], pred_tokens, smoothing_function=self.bleu_smoothing)
                scores.append(score)
    ...
    def calculate_domain_specific_metrics(self, predictions: List[str], references: List[str], 
                                        domain: str = "electric_vehicles") -> Dict[str, float]:
        """Calculate domain-specific metrics"""
        metrics = {}
        
        if domain == "electric_vehicles":
            # EV-specific metrics
            metrics["price_accuracy"] = self._calculate_price_accuracy(predictions, references)
            metrics["technical_accuracy"] = self._calculate_technical_accuracy(predictions, references)
            metrics["compatibility_accuracy"] = self._calculate_compatibility_accuracy(predictions, references)
        
        return metrics
    
    def _calculate_price_accuracy(self, predictions: List[str], references: List[str]) -> float:
    ...
    def _calculate_technical_accuracy(self, predictions: List[str], references: List[str]) -> float:
    ...
    def _calculate_compatibility_accuracy(self, predictions: List[str], references: List[str]) -> float:
        """Calculate compatibility accuracy for EV domain"""
        try:
            compatibility_terms = ["compatible", "adapter", "connector", "CCS", "CHAdeMO", "Type 2"]
            correct_terms = 0
            total_terms = 0
    ...
    def evaluate_model(self, model: Any, tokenizer: Any, benchmark: List[Dict[str, Any]], 
    ...
    def compare_models(self, fine_tuned_model: Any, fine_tuned_tokenizer: Any,
                      baseline_model: Any, baseline_tokenizer: Any,
                      benchmark: List[Dict[str, Any]],
                      fine_tuned_name: str = "fine_tuned",
                      baseline_name: str = "baseline") -> ComparisonResult:
        """
        ...
        for metric in fine_tuned_result.metrics:
            ...
            statistical_significance[metric] = improvement > 0.05
        ...
        evaluation_summary = {
            ...
            "throughput_improvement": fine_tuned_result.throughput - baseline_result.throughput,
    ...
    def save_comparison_results(self, result: ComparisonResult, output_path: str) -> None:
...
"""
        ...
        for metric in result.fine_tuned_metrics:
            if metric in ["rouge1", "rouge2", "rougeL", "bleu", "exact_match", "semantic_similarity"]:
...
"""
        ...
        for metric in result.fine_tuned_metrics:
            if metric not in ["rouge1", "rouge2", "rougeL", "bleu", "exact_match", "semantic_similarity"]:
                ft_score = result.fine_tuned_metrics[metric]
                base_score = result.baseline_metrics[metric]
                improvement = result.improvements[metric]
                relative_imp = result.relative_improvements[metric]
